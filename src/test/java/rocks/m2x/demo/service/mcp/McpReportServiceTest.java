...
class McpReportServiceTest {
    ...
    void testGenerateReportWithTemplateContent() throws Exception {
        ...
        when(templateResourceService.createTemplateFromBase64(any(), anyString(), anyString()))
                .thenReturn(metadata);
        when(templateResourceService.getTemplateResource(metadata.getId())).thenReturn(mockResource);
        ...
        assertNotNull(result);
        verify(templateResourceService).createTemplateFromBase64(any(), anyString(), anyString());
    }
