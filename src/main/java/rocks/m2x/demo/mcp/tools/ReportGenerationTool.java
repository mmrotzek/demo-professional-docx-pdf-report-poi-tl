...
public class ReportGenerationTool {
    ...
    public GenerateReportResult generateReport(
            @McpToolParam(description = "Template identifier (predefined classpath template name or UUID from provide_template). " +
                    "If templateContent is provided, this is optional and used as the template name for metadata.", required = false)
            String templateId,
        ...
        try {
            ...
            log.warn("Security violation in generate_report: {}", e.getMessage());
            return GenerateReportResult.builder()
                    .success(false)
            ...
            log.warn("Invalid request in generate_report: {}", e.getMessage());
            return GenerateReportResult.builder()
                    .success(false)
                    .error("Invalid request")
                    .message("Invalid template or data provided. Please check your request.")
                    .build();
        } catch (Exception e) {
            ...
            return GenerateReportResult.builder()
                    ...
                    .message("Failed to generate report. Please check template and data, then try again.")
                    .build();
        }
    }
