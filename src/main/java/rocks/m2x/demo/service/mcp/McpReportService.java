...
public class McpReportService {
    ...
    public String generateReport(McpReportRequest request) throws IOException {
        ...
        if (request.getTemplateContent() != null && !request.getTemplateContent().isEmpty()) {
            ...
            // Always generates a server-side UUID (templateName is only used for metadata)
            rocks.m2x.demo.mcp.model.TemplateMetadata metadata = templateResourceService.createTemplateFromBase64(
                    request.getTemplateId(), // Used as name for metadata only, not as ID
                    request.getTemplateContent(),
                    "Inline template provided in request");
            // Use the server-generated UUID from metadata
            templateResource = templateResourceService.getTemplateResource(metadata.getId());
